...
#### 16-1. コンソールパネル改善
- [ ] テキスト選択＆コピー対応（`user-select: text`）
- [ ] ログレベルフィルタ（Info / Warn / Error のトグルボタン）
- [ ] クリアボタン ＆ ログ件数表示
- [ ] 自動スクロール制御（新ログ到着時のみ末尾追従）

#### 16-2. エンティティ複製＆コピーペースト
- [ ] `Ctrl+D`: エンティティ複製（子エンティティ＋全コンポーネントのディープコピー）
- [ ] `Ctrl+C` / `Ctrl+V`: コピー＆ペースト（別の親にもペースト可能）
- [ ] `Delete`: エンティティ削除のショートカット
- [ ] 複製時は名前に自動連番 (`Cube` → `Cube (1)`)

#### 16-3. オートセーブ＆復旧
- [ ] IndexedDB への定期オートセーブ（60秒間隔）
- [ ] 起動時の復元ダイアログ（「前回の作業を復元しますか？」）
- [ ] プロジェクト未使用時でもローカルセッションとして保持

#### 16-4. コンポーネントプロパティ Undo/Redo 拡張
- [ ] `PropertyChangeCommand` の汎用実装
- [ ] Inspector の `_emitChange` 経由で自動的にUndoスタックへ積む
- [ ] 対象: マテリアル色/物性値、ライト設定、物理パラメータ、パーティクル設定
...
#### 17-1. 選択ロック＆表示トグル
- [ ] Hierarchy 各行に 👁（表示/非表示）と 🔒（選択ロック）アイコン
- [ ] ロック中エンティティはシーンビューのクリックで選択不可
- [ ] 状態はセッション中維持（シリアライズ対象外）

#### 17-2. スナップ移動＆グリッド設定
- [ ] Toolbar にスナップトグルボタン
- [ ] 移動スナップ: 0.25 / 0.5 / 1m、回転スナップ: 15° / 45° / 90°
- [ ] グリッドサイズ設定 UI
...
#### 18-1. エクスポートパイプライン強化
- [ ] テクスチャ / GLB アセットのエクスポート時パス解決の検証＆修正
- [ ] エクスポート前バリデーション（Missing Asset 警告、Script 構文チェック）
- [ ] ビルドサイズ表示
- [ ] itch.io 向け HTML5 ビルド最適化

#### 18-2. プレビューサーバー
- [ ] ワンクリックでローカルテストサーバーを起動（Vite preview 統合 or 簡易HTTP）
- [ ] QRコード生成 — スマホからの動作確認
- [ ] ホットリロード対応（エクスポートせずにプレイ用ビルドを即座に更新）
...
#### 19-1. テンプレートプロジェクト作成
- [ ] **クリッカーゲーム**: UIテキスト＋ボタン＋プログレスバー＋game.set/get による進捗管理
- [ ] **放置シミュレーション**: 時間経過による自動資源生成＋アップグレード店＋パーティクルエフェクト
- [ ] **3D ビューアー**: GLBモデル表示＋OrbitCamera＋ライティング設定＋ポストプロセス
- [ ] 各テンプレートに `.ludus/README.md` (AI向けプロジェクト概要) を同梱

#### 19-2. テンプレート選択UI
- [ ] エディタ起動時の「スタート画面」（New / Open / Template）
- [ ] テンプレートのサムネイル・説明・ジャンル表示
- [ ] 選択 → プロジェクトフォルダへの自動展開
...
- [ ] エンティティ作成・テンプレート → `EntityFactory.js`
- [ ] キーボードショートカット → `ShortcutManager.js`

#### 20-2. Inspector.js 分割 (95KB → コンポーネント別)
- [ ] 各コンポーネントレンダラーを個別ファイルへ抽出
  - `TransformInspector.js`, `MeshInspector.js`, `PhysicsInspector.js` ...
- [ ] コンポーネントレジストリパターンの導入（新コンポーネント追加時に Inspector を直接編集不要に）

#### 20-3. 型安全性の向上
- [ ] JSDoc の `@typedef` 整備（主要インターフェースの型定義）
- [ ] `tsconfig.json` + `// @ts-check` の段階的導入
...
| ⬜ 14 | **16** | エディタ品質基盤 | デバッグ効率・操作安定性の大幅向上 |
| ⬜ 15 | **17** | エディタ操作効率 | シーン構築のイテレーション速度向上 |
| ⬜ 16 | **18** | エクスポート＆配布 | ゲームを配布・共有可能に |
| ⬜ 17 | **19** | テンプレート＆スターター | 立ち上がり高速化, AI 連携強化 |
| 🔄 — | **20** | コードベース保守性 | 各フェーズと並行して段階実施 |
